...
# https://huggingface.co/docs/hub/security-tokens
hf_token <- readLines("HuggingFace_token.txt")
```
...
```{r}
# label_topics
labels_topicmodels <- label_topics(topics_topicmodels, context = "Journal of Statistical Software", token = hf_token)
...
```{r}
# label_topics: frex
labels_stm_frex <- label_topics(topics_stm, stm_type = "frex", context = "blogposts about American politics", token = hf_token)
...
```{r}
# label_topics
labels_btm <- label_topics(topics_btm, context = "Reviews of AirBnB customers on Brussels address locations", token = hf_token)
...
```{r}
# label_topics
labels_ldaproto <- label_topics(topics_ldaproto, context = "Reuters news data", token = hf_token)
